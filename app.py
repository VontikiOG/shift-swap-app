...
# --- חלון קופץ: יומן שינויים (Changelog) המלא ---
@st.dialog("📜 יומן שינויים - היסטוריית הפיתוח")
def show_changelog():
    st.markdown("""
    ### מה חדש?
    **v1.8.2 | הסלקטור 🚷**
    * מנגנון סינון חכם שמונע משורות סיכום (כמו "משמרת בוקר", "סה"כ", "חופש") להופיע ברשימת העובדים.

    **v1.8 - v1.8.1 | מערכת SaaS ואזור מנהל ☁️🔒**
    * תוקן באג קריסה (`StreamlitDuplicateElementKey`) בהחלפות משולשות.
    * **אזור מנהל מאובטח:** פאנל ניהול נסתר בתפריט הצד, מוגן בסיסמה.
    * **שמירה בשרת המרכזי:** האקסל נשמר בשרת וזמין לכל חברי הצוות בלייב.
    * **תצוגת שבוע אקטיבי:** באנר בראש העמוד שמודיע איזה שבוע מוצג כרגע.
    ...
    * **שכתוב אלגוריתם שעות מנוחה:** בדיקה דו-כיוונית למניעת קומבינות לא חוקיות.
    ...
    * **חיסול המקלדת הקופצת:** מעבר ממקלדת לכפתורי "קפסולות" (Pills).
    * **דילים חכמים לחופש:** החלפה ששומרת על מאזן המשמרות.

    **v1.0 - v1.3 | הבסיס וגרסת האימפריה 👑**
    * מדד עומס, רשימת חרם (Blacklist), ודיווח יבש למנהל.
    * חלונות קופצים וכתיבה שנונה לוואטסאפ.
    """)
