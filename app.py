...
def show_changelog():
    st.markdown("""
    **v1.7.1 | מינימליזם 🧹**
    * הוסרה תצוגת "השבוע שלי" לבקשת המשתמש (פחות עומס ויזואלי).
    * אופטימיזציית Cache לטעינה מהירה.
    * תיקון אלגוריתם שעות מנוחה (בדיקה דו-כיוונית).

    **v1.6 | ההסבר המשולש 🔺**
    * שכתוב הסבר ההחלפה המשולשת בוואטסאפ לשיטת "תן וקח".
    * עיצוב מחדש של חלונית ההסבר ב-HTML.

    **v1.2 - v1.5 | היסטוריית פיתוח 👑**
    * תמיכה מלאה ב-RTL, חלונות קופצים, דילים חכמים לחופש, ומדד עומס.
    """)
...
def get_workload_text(person_name, df):
    ...
    if shifts_count <= 2: return f"🎯 מטרה קלה! ({shifts_count} משמרות)"
    elif shifts_count >= 5: return f"⚠️ קורס/ת מעומס ({shifts_count} משמרות)"
...
def generate_whatsapp_msg(tone, my_shift, partner_shift, day, partner_name):
    msgs = {
        ...
        "פילוסופי": f"היי {partner_name}, ה'איך' של משמרת {my_shift} ב{day} קשוח לי מדי. יש מצב להחלפה תמורת ה{partner_shift} שלך?",
        "איש משפחה במצוקה": f"היי {partner_name}, צץ אילוץ משפחתי ב{day} על ה{my_shift}. יש מצב להתחלף על ה{partner_shift} שלך?",
        "עסקי וקר": f"היי {partner_name}. מתאים להחליף את ה{my_shift} שלי ב{day} ב{partner_shift} שלך?",
        "שוחד": f"עסקה: ה{my_shift} שלי ב{day} אליך, ה{partner_shift} אלי, ולאפה עלי. דיל?",
        "סרקסטי": f"היי {partner_name}, בוא נתחלף ב{day} (שלי {my_shift}, שלך {partner_shift}) כדי שנשאר שפויים. זורם?"
    }
    return msgs.get(tone, "")

def generate_freedom_swap_msg(tone, my_shift, my_day, partner_shift, partner_day, partner_name):
    exp = f"ראיתי שיש לך חופש ב{my_day}. תיקח את ה{my_shift} שלי שם, ואני אקח את ה{partner_shift} שלך ב{partner_day}? ככה החופש שלך פשוט עובר ל{partner_day}."
    if tone == "שוחד": return f"היי {partner_name}, {exp} פלוס קפה ומאפה עלי. סגרנו?"
    return f"היי {partner_name}, {exp} מה אומר?"
...
def find_triangular_swap(user_name, user_shift, selected_day, person_a_name, person_a_shift, df, blacklist):
    ...
    if not valid_bs:
        st.error("אין דיל משולש ריאלי כרגע.")
        return
            ...
            with st.container(border=True):
                ...
                with col_pop:
                    with st.popover("💡 איך זה עובד?"):
                        st.markdown(f"""<div dir="rtl">🟢 <b>אתה:</b> {person_a_shift} ({selected_day})<br>🔵 <b>{person_a_name}:</b> {s} ({d})<br>🟡 <b>{b_name}:</b> {user_shift} ({selected_day})</div>""", unsafe_allow_html=True)

def main():
    st.title("מערכת חילופי משמרות 🔄")
    col_v, col_b = st.columns([2, 1])
    with col_v: st.caption("v1.7.1 | גרסת המינימליזם 🧹")
    with col_b: 
    ...
    if uploaded_file:
        try:
            ...
            user_name = st.pills("מי אתה?", workers, selection_mode="single")
            
            if user_name:
                user_shifts = {d: s for d, s in df[df['שם'] == user_name].iloc[0].to_dict().items() if d != 'שם' and s != 'חופש 🌴'}
                if not user_shifts:
                    st.success("אין לך משמרות! תהנה בחופש. 🏖️")
                else:
                    day = st.pills("מאיזו משמרת לברוח?", list(user_shifts.keys()), selection_mode="single")
                    if day:
                        curr_s = user_shifts[day]
                        blacklist = st.pills("מי לסנן? (חרם):", [w for w in workers if w != user_name], selection_mode="multi") or []
                        wanted = st.pills("מה אתה מעדיף במקום?", ["בוקר ☀️", "בוקר ארוך 🌤️", "ערב 🌇", "לילה ארוך 🦉", "לילה 🌙", "חופש 🌴"], selection_mode="multi")
                        ...
                        if wanted:
                                ...
                                for _, p in free.iterrows():
                                    ...
                                    if check_legal_rest(p_n, curr_s, day, df):
                                        ...
                                        if opts:
                                            found = True
                                            with st.container(border=True):
                                                st.markdown(f"### 🌴 {p_n} (חופשי ב{day})")
                                                idx = st.radio("מה תיקח לו?", range(len(opts)), format_func=lambda x: f"{opts[x][1]} ב{opts[x][0]}", key=f"c_{p_n}", horizontal=True)
                            ...
                            if not found: st.error("אין פראיירים פנויים. ☕")
